...
- Field **type** is whatever `GetTypeName()` returns, which is the *pointee's* name — indirection is reported separately via the `pointerLevel` out-parameter, so `get_struct_single` appends that many `*`. `GetTypeName()` can return `nullptr` (e.g. for `LF_PROCEDURE`); never build a `std::string` from it unchecked.
- `bitfield_offset` is `LF_BITFIELD`'s `position`, 0 for a normal member. The record also carries `length` (the member's width in bits), which is deliberately not exposed.
